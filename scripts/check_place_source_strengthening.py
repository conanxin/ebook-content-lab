#!/usr/bin/env python3
"""Validate v0.7-A12 place-source strengthening."""
...
def scan_json(data: Any, target: str, findings: list[Finding]) -> None:
    ...
    hits = [marker for marker in SOURCE_MARKERS if marker.lower() in text]
    if hits:
...
def check_content(payloads: dict[str, dict[str, Any]], findings: list[Finding]) -> dict[str, int]:
    ...
    stats = overview.get("place_source_stats", {})

    summary_count = sum(1 for item in chapters if str(item.get("source_informed_summary") or "").strip())
    excerpt_count = sum(1 for item in chapters if item.get("original_excerpt"))
    then_now_count = sum(1 for item in chapters if str(item.get("then_now_comparison") or "").strip())
    answer_count = sum(1 for item in questions if str(item.get("answer_hint_expanded") or item.get("answer_hint") or "").strip())
    ...
    if summary_count != 25:
        add_error(findings, "source_informed_summary", f"Expected 25/25, got {summary_count}")
    if excerpt_count != 25:
        add_error(findings, "original_excerpt", f"Expected 25/25, got {excerpt_count}")
    if then_now_count != 25:
        add_error(findings, "then_now_comparison", f"Expected 25/25, got {then_now_count}")
    if answer_count != 26:
        add_error(findings, "reading question answers", f"Expected 26/26, got {answer_count}")

    if public_source_count <= A11_PUBLIC_SOURCE_COUNT:
        add_error(findings, "place_then_now", f"public_source count did not increase from A11 baseline {A11_PUBLIC_SOURCE_COUNT}")
    if public_source_count < 20:
        add_warning(findings, "SOURCE_COVERAGE_LOW", f"public_source count below target: {public_source_count}")
    if stats.get("public_source_count") != public_source_count and stats.get("a12_public_source_count") != public_source_count:
        add_error(findings, "place_source_stats", "Stats public_source count does not match place list")

    for index, item in enumerate(places):
        target = f"place_then_now[{index}]"
        if item.get("source_status") == "public_source":
            for field in ["source_name", "source_url", "source_type", "source_review_note", "updated_in"]:
                if not item.get(field):
                    add_error(findings, f"{target}.{field}", "public_source entry missing required source field")
        elif item.get("source_status") == "needs_source_review":
            if not item.get("source_review_note"):
                add_error(findings, f"{target}.source_review_note", "needs_source_review entry missing review note")
        else:
            add_error(findings, f"{target}.source_status", f"Unexpected source_status: {item.get('source_status')!r}")

    route_index = overview.get("route_index", [])
    if len(route_index) != 25:
        add_error(findings, "route_index", f"Expected 25 route index entries, got {len(route_index)}")
...
def check_page(paths, findings: list[Finding]) -> None:
    ...
    for class_name in [
        ...
        if class_name not in page_text or class_name not in styles_text:
            add_error(findings, class_name, "A12 UI class not wired in page and CSS")
...
def render_report(version: str, metrics: dict[str, int], manual: dict[str, int], findings: list[Finding]) -> str:
    ...
    lines = [
        f"# Place Source Strengthening Validation {version}",
        "",
        f"- Status: `{status}`",
        f"- Errors: `{len(errors)}`",
        f"- Warnings: `{len(warnings)}`",
        f"- Chapter cards: `{metrics.get('chapters', 0)}`",
        f"- Reading questions: `{metrics.get('questions', 0)}`",
        f"- Source-informed summaries: `{metrics.get('summary_count', 0)}/25`",
        f"- Original excerpts / clues: `{metrics.get('excerpt_count', 0)}/25`",
        f"- Then/now comparisons: `{metrics.get('then_now_count', 0)}/25`",
        f"- Enhanced answers: `{metrics.get('answer_count', 0)}/26`",
        ...
        f"- Route index entries: `{metrics.get('route_index_count', 0)}`",
...
def main() -> int:
    ...
    report_path = paths.report_path(f"place_source_strengthening_validation_{normalize_version(args.version)}.md")
