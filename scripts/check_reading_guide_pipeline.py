...
def run_inspect(paths: ProjectPaths) -> tuple[dict[str, Any] | None, str | None]:
    script = paths.repo_root / "scripts" / "inspect_reading_guide_pipeline.py"
    result = subprocess.run(
        [sys.executable, str(script), "--project", str(paths.project_dir)],
        cwd=paths.repo_root,
        capture_output=True,
        text=True,
        encoding="utf-8",
...
def scan_public_json(paths: ProjectPaths, findings: list[Finding]) -> None:
    public_roots = [
        paths.public_dir,
        paths.web_project_dir,
    ]
        ...
        for path in base.rglob("*"):
            ...
            for pattern in PUBLIC_SCAN_PATTERNS:
                if pattern.lower() in lowered:
                    add(findings, "error", path, f"Public file contains private/full-text marker `{pattern}`.", paths.repo_root)


def scan_reports_for_body_text(paths: ProjectPaths, findings: list[Finding]) -> None:
    ...
    for path in reports_dir.glob("*.md"):
        ...
        for pattern in REPORT_BODY_PATTERNS:
            if pattern.lower() in lowered:
                add(findings, "error", path, f"Report contains body-text marker `{pattern}`.", paths.repo_root)
        for line_no, line in enumerate(text.splitlines(), start=1):
            stripped = line.strip()
            if len(stripped) > 1000:
                add(findings, "warning", path, f"Line {line_no} is over 1000 chars; review for long excerpt risk.", paths.repo_root)
...
def check_project(paths: ProjectPaths, version: str) -> tuple[list[Finding], dict[str, Any] | None]:
    repo_root = paths.repo_root
    findings: list[Finding] = []

    inspect_data, inspect_error = run_inspect(paths)
    if inspect_error:
    ...
    required_path(paths.book_identity_json, "working/book_identity.json", findings, repo_root)
    required_path(paths.book_identity_source_json, "working/book_identity_source.json", findings, repo_root)
    required_path(paths.book_identity_report, "reports/book_identity_report.md", findings, repo_root)
    required_path(paths.epub_extraction_report, "reports/epub_extraction_report.md", findings, repo_root)

    scan_public_json(paths, findings)
    scan_reports_for_body_text(paths, findings)

    if inspect_data:
        if not inspect_data.get("intake_ready"):
            add(findings, "error", paths.project_dir, "inspect reports intake_ready=false.", repo_root)
        if not inspect_data.get("privacy_boundary_ok"):
            add(findings, "error", paths.project_dir, "inspect reports privacy_boundary_ok=false.", repo_root)
...
def main() -> int:
    ...
    paths = ProjectPaths.from_project(args.project)
    findings, inspect_data = check_project(paths, args.version)
    report_path = paths.pipeline_check_report(args.version)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(
        render_report(paths.project_dir, args.version, inspect_data, findings, paths.repo_root),
        encoding="utf-8",
    )
